...
public class agent{
    ...
    int getAction(state curState){
        ...
        for (int i = 1; i < 5; i++) {
          if (nAct[i-1] < Ne)
            temp = rn.nextInt(100); //Make the temp as large as possible so that it will try the current position
          else
            temp = Q[i-1];
    ...
    state takeAction(int dir) {
      ...
      if(board.feature[cur.i][cur.j] == 'H') {
        exponent = 1.0;
      }
    ...
    void tdQlearning(){
        ...
        for(int l = 0; l < 1000000000; l++){
          ...
          //Update Q(s,a)
          if(board.feature[cur.i][cur.j] == '*') {
            curQ[act-1] = curQ[act-1] + alpha * (curReward * exponent + gamma * maxQ - curQ[act-1]);
            exponent += .1;
          } else {
